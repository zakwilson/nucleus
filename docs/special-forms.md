...
| `as` | **Statically-safe** conversion — `(as TYPE expr)`, same shape as `unsafe/cast`. Accepts the *non-lossy* part of what the implicit-coercion machinery accepts in an assignment position (identity, int widening, same-width sign reinterpret, `f32`→`f64`, user `defcast` rules, `CStr`↔pointer, the elem-less-`ptr` `void*` hatch) **plus** pure pointer-contract weakening (`(ref T)`→`(raw T)`; a typed pointer → elem-less bare `ptr`) **plus** an integer narrowing whose operand is a *literal* that fits the target — `(as i8 5)`, `(as ui8 200)`, and the same through a `defconst` name — which is lossless and so lowers to exactly what the implicit spelling `(let (a:i8 5) …)` emits. Refuses everything lossy or contract-manufacturing — narrowing/truncation of a **value** (integer *and* `f64`→`f32`), a literal that does *not* fit (`(as i8 300)`, `(as ui8 -1)`), `float`→`int`, `ptr`↔`int`, `fn`↔`ptr`, element-retyping `ptr`↔`ptr` (each routed to `unsafe/cast`) — note that an implicit coercion at a typed slot narrows a *value* silently (integers and floats alike), so `as` stays deliberately stricter than assignment there — and, unlike `unsafe/cast`, **honors the nullability flow check**: a `raw`/nullable pointer into a non-null `(ref T)`/`(ptr T)` slot is rejected (routed to `as-ref` for a runtime-checked launder, or `unsafe/cast` for an unchecked assertion). Use `as` for conversions you can prove correct; reach for `unsafe/cast` only when the compiler refuses. **`as` also names a type for inference**: its target type is supplied as the expected type of its operand, exactly as a `let`/`with` binding annotation, a `set!` target, a `return` or a `.set!` field does — so a generic whose type variable appears only in its return type resolves against it (`(as (ref (Vector i32)) (vector-new-in a))`). | `static_cast`-ish |
