...
**`?`/`!` in names.** A `defn`/method name may use `?` and `!`, matching the convention of the builtins `set!`/`inc!`/`empty?`/`contains?` (e.g. `(defn full? …)`, `(defn push! …)`). In the emitted symbol each `?` becomes `_QMARK` and each `!` becomes `_BANG` — `full?` → `@full_QMARK`, `push!` → `@push_BANG` — for both solitary and overloaded names; every other character (hyphens included) is untouched. Because the mapping is textual, a name ending in the literal substring `_QMARK` collides with one ending in `?` (likewise `_BANG`/`!`) — a narrow, unlikely-in-practice edge case, not diagnosed. Struct and union type names follow the same mapping (`(defstruct Full? …)` → `%Full_QMARK`). See `examples/predicate-names.nuc` and `examples/predicate-types.nuc`.

**Every position, not only the exported ones.** The mapping applies wherever a Nucleus name becomes an LLVM identifier — a **parameter**, a `let` or `with` binding, a `match` binder, and a `label`/`goto` target as well as a `defn`, `defvar` or type name. The rule is about becoming an LLVM name, not about being a linked one, so `(defn add? (ok?:i32 n!:i32):i32 (let (acc!:i32 …) …))` compiles like any other function. The source name is what stays in scope: `scope-define` keys on `ok?`, and only the emitted `%ok_QMARK.addr` slot carries the escape.

**A leading digit.** LLVM's unquoted identifier is `[%@][-a-zA-Z$._][-a-zA-Z$._0-9]*` — a character class for the body *and* a rule about the first position: it may not be a digit. A name that starts with one therefore takes a single leading `_` in the emitted symbol, in every position above: `(defn 2fast …)` → `@_2fast`, `(defstruct 2Pair …)` → `%_2Pair`, `(defvar 2count:i32 …)` → `@_2count`. C has the same restriction, so a generated header spells the same escape — `int32_t _2fast(int32_t _2n);` with no `asm()` label, because the C spelling and the link symbol agree. The collision hazard is the `_QMARK` one exactly: a name written `_2fast` in source now shares the symbol, and is not diagnosed.
...
**A protocol's *methods* can be qualified too.** A protocol method name is an ordinary overloaded name, so it follows the rule in [Qualifying an overloaded function](toplevel.md#qualifying-an-overloaded-function): there is one entry per bare name with every namespace's methods merged, and a reference *filters* it. With `(import-prefixed thatlib p)`, `(p/describe x)` calls only the `describe` methods `thatlib`'s namespace declared — which is how two libraries that each declare a `describe` stay usable in one file without routing every call through a vtable. A **bare** `(describe x)` filters too, to the namespaces this file can name unqualified: it sees `thatlib`'s methods only if the file flattened it with `import-use`, and a prefixed import contributes nothing to it. Within that set the call resolves by argument type as usual.

A method reached through a `(dyn P)` box is unaffected by any of this: the box dispatches through its vtable, so `(describe b)` on a `(dyn p/Describe)` works whatever the file's import environment is — the conformance was recorded where the type and the protocol are both nameable.
