...
| `--emit-nuch` | Output a `.nuch` header instead of compiling. Extracts function signatures, struct definitions, constants, enums, and macros. The prelude and the file's own imports are prescanned first (types only — nothing is emitted), so an exported signature may name an imported type: `Node`, `StrView`, `String`, `(Maybe T)`, the `!T` sugar. Declarations are validated before anything is written (see [What a header mode checks](compiler.md#what-a-header-mode-checks)). |
| `--emit-cheader` | Output a C header (`.h`) instead of compiling. Emits `#pragma once`, `#include <stdint.h>`, tagged typedefs for structs, extern function declarations (an overloaded or operator-named function under the per-signature symbol it links as), `extern` declarations for `defvar` and `extern` globals, `#define` constants, and enums. A name C or C++ reserves (`union`, `class`) is renamed with a trailing `_` and re-bound with an `asm` label. Resolves the unit's signatures and validates its declarations, so a source whose *declarations* do not compile produces the compiler's ordinary error rather than a header; a body error is not detected in this mode (see [What a header mode checks](compiler.md#what-a-header-mode-checks)). |
...
path applies only to types that do **not** define `invoke` (which outranks `get`).
...
with the argument(s) taken as values — so the callee cannot be used for field
access by call. Dispatch is ordinary multimethod resolution on the whole argument
tuple:
...
- **Integer ↔ integer**:
  ...
  - Narrowing: `trunc` — except that an out-of-range integer *literal* is a compile-time error rather than a silent wrap.
- **Float ↔ float**: `fpext` widening; `fptrunc` narrowing for a value, and no instruction at all for a float *literal* (it is re-rendered as a single-precision constant at compile time).
...
Explicit `(unsafe/cast ...)` is also still required for cross-kind conversions: `int ↔ ptr`, `int ↔ float`, and `ptr ↔ float`. Narrowing a *value* is likewise implicit-only — the explicit `as` refuses it. A narrowing **literal** the target can hold exactly is the exception in both directions: it is lossless, so `(as i8 5)` is accepted and emits exactly what `(let (a:i8 5) …)` emits, and so is `(as f32 1.5)`. The float rule is stricter in one respect, deliberately: `as` admits a float literal only when it round-trips **exactly**, so `(as f32 3.14)` is still refused although `(let (a:f32 3.14) …)` compiles — the implicit path rounds to the nearest single silently, and `as` is the spelling that promises nothing was lost.
