...
Supported forms: `declare` (function signatures), `defstruct`, `defconst`, `defenum`, `defmacro` (full body preserved), `defcast` (full form preserved — the conv-fn must already be `declare`d in the same header), `extern` (foreign global variables — re-emitted from a producing module's `defvar` so importers see the symbol without its initializer).
