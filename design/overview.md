...
- [stage11/assoc-types.md](stage11/assoc-types.md) — **design only (not yet implemented)** — associated types for protocols: `(type Elem)` declaration in `defprotocol`; binding via existing `(extend T (Protocol A))` form; `&where ((Iterator I) with (Elem = E))` syntax to derive the associated type in generic functions; changes to `g-conformances` registry, `emit-extend`, `&where` parser, `unify-tpat`, and `register-generic-defn`; migration path from phantom-param verbose forms; out of scope (multiple assoc types, HKT, lambdas); three open questions (ambiguity, import/re-stamp, return-position tyvars)
