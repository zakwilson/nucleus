...
| 14 (macro casts) | Macro conditionals without casts — shared `type-join` absorbing elem-less `ptr` into the typed side at pkind `raw` (MC-1), quasiquote/`gensym` results retyped `(raw Node)` (MC-2), cast deletion in `lib/macros.nuc` + castless test (MC-3), docs sweep (MC-4) | **MC-1 done** — `type-join` helper added to `src/generics.nuc` (after `type-eq`; must call `type-eq` so can't live in `type-utils.nuc`); three join sites in `emit-cond`/`emit-match`/`emit-niche-match-arm` call it; byte-identical bootstrap (no bare-vs-typed join in compiler source); 139 tests pass. MC-2 → MC-3 → MC-4 remain. | [stage14/macro-conditional-casts.md](stage14/macro-conditional-casts.md) |
