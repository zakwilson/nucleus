...
| 14 (macro casts) | Macro conditionals without casts — shared `type-join` absorbing elem-less `ptr` into the typed side at pkind `raw` (MC-1), quasiquote/`gensym` results retyped `(raw Node)` (MC-2), cast deletion in `lib/macros.nuc` + castless test (MC-3), docs sweep (MC-4) | **MC-1 + MC-2 done** — `type-join` in `src/generics.nuc` absorbs bare↔typed joins (MC-1, byte-identical); `ty-raw-node` accessor + retyped `emit-quote`/`emit-qq-list`/`emit-qq-form`/`gensym` + `node-type-call` lockstep (MC-2, required bootstrap reconverge); 139 tests pass. MC-3 (cast deletions + castless test) → MC-4 (docs) remain. | [stage14/macro-conditional-casts.md](stage14/macro-conditional-casts.md) |
