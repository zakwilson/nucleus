...
(Status: the PassBuilder FFI is declared in `src/llvm.nuch`, but `compile-and-link` does **not** yet call `LLVMRunPasses` — `-O`N` only sets the codegen (instruction-selection / regalloc) level via `LLVMCreateTargetMachine`. Wiring the middle-end pipeline is the open work.)
