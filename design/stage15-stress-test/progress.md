...
| 28 | **A C keyword is emitted as an identifier**, found fixing item 4 | *(Still open. Item 26 removed its only corpus instance without touching the defect: `union` in `lib/hashset.nuc` is a template method, so it is now skipped rather than declared. Re-confirmed on a fresh probe — a non-template `(defn union (a:i32 b:i32):i32 …)` still emits `int32_t union(int32_t a, int32_t b);`. Latent, not fixed.)* `lib/hashset.nuc` defines `union`, a perfectly ordinary Nucleus name, and the header emits `void union(void* self, void* other);`. `sanitize-for-c` maps illegal *characters* and has no notion of a reserved word, so the name passes through intact and the header does not parse. The same applies to `int`, `return`, `default`, `switch`, `class` and the rest. Item 4's asm-label mechanism makes this trivially fixable (rename to `union_` and label it `asm("union")`, exactly as a hyphenated name is handled) but it is a naming ruling, not a missed call site |
