...
> initialization — its **G-0** and **G-1** steps are done, 2026-08-01 and
> 2026-08-02 respectively; G-2 onward not started) and **W9** (ten defects
> found while measuring W8's design, four of them added by G-1's own
> measurement — reported, not fixed). Nothing about W1–W7 changed; the stage
> simply has more in it. Current verified state: **361 tests**, `make
> bootstrap` byte-identical, `make abi-test`/`make layout-test` green.
...
[stage14/attributes.md](../stage14/attributes.md). **Note that W8's G-2 takes
fixed-size arrays out of that deferral**: an `(array T N)` type is the same
feature at a `defvar` and at a `defstruct` field, and `global-init.md` §5 says
to do them together.
