...
> **Stage status, 2026-08-02: reopened scope complete.**
> **W1–W7 are done and their record stands unchanged** — 328 tests,
> `make bootstrap` byte-identical, both Doom demo gates bit-exact
> ([prompt.md](prompt.md) §0, §6). Two new items were added to the stage by
> the author's direction and are **both now done**: **W8** (combined
> declaration and initialization — all six steps **G-0 through G-5** landed,
> 2026-08-01/2026-08-02, plus one regression fix between G-3 and G-4;
> `compiler-init` is eliminated and `(defvar g:ptr:T)` with no initializer is
> now rejected, closing [nullability.md](nullability.md) §1.5's remaining
> half) and **W9** (pre-existing defects found while measuring W8's design —
> reconciled at stage close to **twenty found, two now fixed, eighteen
> open**; reported except where fixed). Nothing about W1–W7 changed; the
> stage simply has more in it. Current verified state: **410 tests**
> (counted with `NUCLEUS_TEST_JOBS=1`), `make bootstrap` reconverged and
> passing, `make abi-test`/`make layout-test`/`make avr-test` green.
...
| **W6** | Nullability flow typing (flow typing itself is still **design only**; the §3.4 triage item **landed** — a `null` global initializer into a typed non-null pointer (`ptr:T` / `(ref T)`) is now rejected with the same diagnostic the identical *local* gets. `defvar-init-ir` is a constant renderer outside the `coerce-int-val` chokepoint, so it never ran `pkind-flow-check`; it now calls that same predicate with `ty-raw` as the source, which is what makes the carve-outs automatic — `CStr`, elem-less bare `ptr`, `(raw T)` and `?T` all stay legal. `emit-defvar`'s no-init sibling is the same unsoundness in another spelling and is deliberately still open: 53 such globals exist in the compiler's own source, so closing it is a deferred-initialization language question rather than a check) | §3.3 §3.4 | Design | [nullability.md](nullability.md) |
...
| **W8** | Combined declaration and initialization (**done — all six steps G-0 through G-5 landed, 2026-08-01/2026-08-02**). Spec: **[../global-init.md](../global-init.md)** — the source of truth; this table does not restate it. Headline goal: **eliminate `compiler-init`, or reduce it to a few genuinely special cases**; a global that should not be nullable is declared and initialized in **one** operation; and the startup call is **zero-cost when unused** (a program with no runtime initializer emits no `@__nucleus_init`, no `llvm.global_ctors` entry, no synthesized `main` — a hard requirement, for microcontroller binary size). Closes the half of W6 §1.5 that item deliberately parked: `(defvar g:ptr:T)` now rejects with a located error instead of silently emitting an unsound `global ptr null`. Staged **G-0 done** (value names resolve on reachability — shippable alone, and it fixes the Doom-port regression's second finding; all four §2.5 probes now resolve both orders, tests 328→347) → **G-1 done** (the static half's constant-folding piece: `+ - * / %`/bit-ops/`sizeof`/`as`/`addr-of` fold with no JIT in `defvar-init-ir`, superseding W5c's `(as CStr …)` out-of-scope note; tests 347→361) → **G-2 done** (an `(array T N)` type usable as both a `defvar` type and a `defstruct`/anonymous-struct field type — the port's §4.2 Major finding — plus constant aggregate initializers, decaying to `(ref T)` on read; the `g-arena-alloc` conversion was split out and de-risked, since it cannot coexist with byte-identical bootstrap at that point; tests 361→385) → **G-3 done** (`@__nucleus_init`, emitted only when the queue is non-empty, registered by `llvm.global_ctors` on hosted triples and refused with a located error on AVR; proven zero-cost by a two-sided tripwire; tests 385→393) → **an interlude** (a G-0 regression that made a function-pointer-typed `defvar` uncollectable, fixed; tests 393→396) → **G-4 done** (the ordering diagnostic: a `defvar` initializer naming a global whose own `defvar` has not been reached is a located error naming both sites; tests 396→406) → **G-5 done** (**acceptance criterion (A)**: `compiler-init` goes from 52 statements to zero, all 48 globals converted; **acceptance criterion (B)**, the flip: `(defvar g:ptr:T)` with no initializer is now rejected, `ptr:T` means non-null at a global exactly as everywhere else; tests 406→410). `make test` **410/410** (`NUCLEUS_TEST_JOBS=1`), `make bootstrap` reconverged and passing (G-5 deliberately moved the compiler's own IR — proven a fixed point independently of any committed artifact), `make abi-test`/`make layout-test`/`make avr-test` green — see [progress.md](progress.md)'s "G-0 through G-5 as built" for the full account. | §3.4 §4.2 (and W6 §1.5) | Large | [../global-init.md](../global-init.md) |
| **W9** | Pre-existing defects found while measuring W8's design (**reported except where fixed; reconciled at stage close to twenty found, two now fixed, eighteen open**). `[progress.md](progress.md)`'s W9 table is the reconciled list — its items 1–13 are the pass recorded before G-3/G-4/G-5, and items 14–20 fold in `global-init.md` §7's later additions (#9–#12, from building G-3 and G-5) plus three defects that were never filed in either list, found alongside the fn-pointer interlude and documented only in `examples/fnptr-global.nuc`'s comments. **Two are now fixed**: the fn-pointer-typed `defvar` collision (a G-0 regression, fixed in the interlude) and `emit-as` not arming the want channel for a return-only-tyvar generic (fixed in G-5). Eighteen remain open, in short: `make lib-objs`/`make lib-so` broken on the committed boot; two separately compiled Nucleus objects unlinkable (7 duplicate prelude globals); `--emit-cheader` does not export globals; `--emit-cheader` emits hyphenated, invalid C identifiers (**a missed `sanitize-for-c` call site, not a missing mechanism**); `(exclude-prelude)` in an *imported* file dies `unknown top-level form`; the misleading `not defined anywhere in this compilation unit` message (**closed at the cause by G-0** for the import shapes it covers, not filed twice); `pkind-flow-check`'s `CStr` carve-out accepts a null through a non-null `(ref T)`; `emit-as` ignores a literal's value when checking int-narrowing (a different `emit-as` defect than the fixed one above); `(defvar g:i1 5)` emits `global i1 5`, silently truncated to `true` by LLVM; `tests/run-tests.sh`'s parallel unit output can interleave, making its PASS count unreliable by ±1; three `fmt-s` call sites pass two substitutions to a one-argument helper, segfaulting the compiler with no output if ever hit; a wrong-arity call to a solitary (non-overloaded) `defn` is not diagnosed at all; `parse-type-from-node`'s diagnostic is unreachable for an unrecognized type-expression head; `aref` emits a hardcoded `i64` GEP index, invalid IR on AVR; a `defvar` may be declared twice in one unit with no diagnostic; `(= h null)` does not compile for a fn-pointer value (`emit-binop-vals` gates on `is-ptr-like`, which excludes `TY-FN`); `type-size` has no `TY-FN` case, so a fn-pointer slot emits `align 1` (conservative, not a miscompile); and `coerce-int-val` has no `raw`→`TY-FN` case, so `(let (f:(fn i32)(i32) null) …)` still dies though the identical `defvar` now compiles. | — | Small–Medium | [../global-init.md](../global-init.md) §7, [progress.md](progress.md) |

Deferred to a later stage, with reasons, in [prompt.md](prompt.md) §7: struct
packing (§4.1) remains deferred — it wants a layout-attribute design that
overlaps [stage14/attributes.md](../stage14/attributes.md), and G-2
**measured, rather than assumed, that it needs nothing from that design**: all
six of G-2's layout shapes and four of its ABI shapes match the platform C
compiler with natural alignment alone. **Fixed-size array fields (§4.2) are no
longer deferred**: W8's **G-2** (done 2026-08-02) delivered `(array T N)` as
one feature at both a `defvar` and a `defstruct` field, per `global-init.md`
§5's instruction to do them together.
...
**W8 and W9 were added after W1–W7 closed**, so they sit after all of them.
Within the pair, W9's items are independent of each other and of W8 with one
exception — W9's sixth defect is caused by the same gap W8's **G-0** closes, so
G-0 fixes it properly and the interim response is a W1c-style note on the
message, not a separate mechanism. W8's own internal order is `global-init.md`
§5's G-0 → G-1/G-2 → G-3 → G-4 → G-5, and G-0 is shippable on its own.
