...
> **Stage status, 2026-08-02: reopened with new scope, in progress.**
> **W1–W7 are done and their record stands unchanged** — 328 tests,
> `make bootstrap` byte-identical, both Doom demo gates bit-exact
> ([prompt.md](prompt.md) §0, §6). Two new items have since been added to the
> stage by the author's direction: **W8** (combined declaration and
> initialization — its **G-0**, **G-1** and **G-2** steps are done, 2026-08-01
> and 2026-08-02 (twice); G-3 onward not started) and **W9** (thirteen
> defects found while measuring W8's design, seven of them added by G-1's and
> G-2's own measurement — reported, not fixed). Nothing about W1–W7 changed;
> the stage simply has more in it. Current verified state: **385 tests**
> (counted with `NUCLEUS_TEST_JOBS=1`), `make bootstrap` byte-identical, `make
> abi-test`/`make layout-test` green.
...
| **W8** | Combined declaration and initialization (**G-0 done 2026-08-01, G-1 and G-2 done 2026-08-02; G-3 onward not started**). Spec: **[../global-init.md](../global-init.md)** — the source of truth; this table does not restate it. Headline goal: **eliminate `compiler-init`, or reduce it to a few genuinely special cases**; a global that should not be nullable is declared and initialized in **one** operation; and the startup call is **zero-cost when unused** (a program with no runtime initializer emits no `@__nucleus_init`, no `llvm.global_ctors` entry, no synthesized `main` — a hard requirement, for microcontroller binary size). Closes the half of W6 §1.5 that item deliberately parked: `(defvar g:ptr:T)` still emits an unsound `global ptr null`, and W6 could not close it because the language has no way to express deferred initialization of a non-null global. Staged **G-0 done** (value names resolve on reachability — shippable alone, and it fixes the Doom-port regression's second finding; all four §2.5 probes now resolve both orders, tests 328→347) → **G-1 done** (the static half's constant-folding piece: `+ - * / %`/bit-ops/`sizeof`/`as`/`addr-of` fold with no JIT in `defvar-init-ir`, superseding W5c's `(as CStr …)` out-of-scope note; tests 347→361, bootstrap byte-identical on the first pass) → **G-2 done** (the remaining static-half piece: an `(array T N)` type usable as both a `defvar` type and a `defstruct`/anonymous-struct field type — the port's §4.2 Major finding — plus constant aggregate initializers; an array is a VALUE that DECAYS to `(ref T)` on read, C's model, gated by one consumed-once permission rather than N per-position refusals; the `g-arena-alloc` conversion the spec demanded was split out and de-risked rather than built, since it cannot coexist with byte-identical bootstrap; tests 361→385, bootstrap byte-identical on the first pass) → G-3 (`@__nucleus_init`, emitted only when non-empty) → G-4 (ordering diagnostic + docs) → G-5 (eliminate `compiler-init`, then flip). `make test` 385/385 (`NUCLEUS_TEST_JOBS=1`), `make bootstrap` byte-identical, `make abi-test`/`make layout-test` green — see [progress.md](progress.md)'s "G-0, G-1 and G-2 as built" for the full account. | §3.4 §4.2 (and W6 §1.5) | Large | [../global-init.md](../global-init.md) |
| **W9** | Thirteen pre-existing defects found while measuring W8's design (**reported, not fixed**). The original six are enumerated in [../global-init.md](../global-init.md) §7; four more (7–10) surfaced measuring G-1 and re-verifying G-0/G-1's test counts, recorded in [progress.md](progress.md) only; two more (11–12) surfaced building G-2 and are also `global-init.md` §7's own #7/#8 (a matched pair); a final one (13) surfaced closing out this stage record, recorded in [progress.md](progress.md) only. In short: `make lib-objs`/`make lib-so` broken on the committed boot; two separately compiled Nucleus objects unlinkable (7 duplicate prelude globals); `--emit-cheader` does not export globals; `--emit-cheader` emits hyphenated, invalid C identifiers (**a missed `sanitize-for-c` call site, not a missing mechanism** — it reaches type names but not field names or function names); `(exclude-prelude)` in an *imported* file dies `unknown top-level form`; the misleading `not defined anywhere in this compilation unit` message (**closed at the cause by G-0** for the import shapes it covers, not filed twice); `pkind-flow-check`'s `CStr` carve-out accepts a null through a non-null `(ref T)`, in both a `defvar` initializer and the identical local; `emit-as` ignores a literal's value when checking int-narrowing, so `(as i8 5)` is refused though it fits; `(defvar g:i1 5)` emits `global i1 5`, silently truncated to `true` by LLVM; `tests/run-tests.sh`'s parallel unit output can interleave, making its PASS count unreliable by ±1 (FAIL detection unaffected); three `fmt-s` call sites at cold diagnostic paths pass two substitutions to a one-argument helper, segfaulting the compiler with no output if ever hit; a wrong-arity call to a solitary (non-overloaded) `defn` is not diagnosed at all, very likely why those `fmt-s` sites have never been hit; and `parse-type-from-node`'s `unable to parse type expression` diagnostic is unreachable for an unrecognized type-expression head, surfacing instead as a misleading `missing :type`. | — | Small–Medium | [../global-init.md](../global-init.md) §7, [progress.md](progress.md) |
