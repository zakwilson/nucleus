...
- Macros that reference `*Node`, `NODE-CELL`, `NODE-SYM` etc. (like `->` and `dotimes`) need the `Node` type in scope, which the prelude provides via `(import node)`. A macro body may also **call ordinary program `defn`s** (e.g. `node-at`, `intern-symbol`): the macro/CT JIT module re-`declare`s each one so the symbol resolves from the `-rdynamic` host binary at JIT-link time. This is automatic — `emit-defn` records every main-module defn's signature (`g-program-defns`/`ProgDefn`) and `emit-call-with-args` emits the declare into the JIT module on first reference (`macro-jit-ensure-decl`). Only main-module defns are re-declared; externs/`(include …)` declarations are already in `g-decl-bufp` (copied into the JIT module), and a defn *defined inside* a CT block is skipped (it already lives in that module). Note: `abi-print-param` writes to `g-out` (the def stream), so the declare's param types are formatted straight into `g-decl-out` instead.
