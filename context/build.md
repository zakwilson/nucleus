...
- The compiler emits **opaque-pointer** LLVM IR (`ptr`, not `i8*`/`i32*`). The target triple + datalayout come from LLVM at startup (host by default; overridable with `--target=<triple>`), so emitted IR runs on whatever host LLVM was built for. Backends for non-host triples must be registered before they can be selected; `targets-init-all` (called from both `target-init` and `jit-ensure-init`) registers X86, AArch64, and ARM, so `--target=` resolves for x86_64/i386/aarch64/arm across Linux/Darwin/Windows triples.
- JIT-bound modules (compile-time bodies, `defmacro`, REPL eval) always use the host triple via `emit-jit-module-target`, even when `--target=` selects a different output triple — the JIT runs in this process, so its modules must match host layout. The final batch output module uses `emit-output-module-target`, which honors `--target=`.
...
- **Optimization (`-O1+`, `-ffast-math`/`-Ofast`, `-march=native`) runs only on the object/binary path** (`compile-and-link` calls `LLVMRunPasses` with `default<O`N`>` before `LLVMTargetMachineEmitToFile`). `--emit-llvm` always emits **unoptimized** textual IR — there is no flag to dump post-pass IR (would need a `LLVMPrintModuleToString` after `LLVMRunPasses`). Consequence: to inspect what the optimizer did, compile to `.o`/binary and `objdump -d`. This also means the bootstrap fixed-point check (which goes through `--emit-llvm`) is structurally immune to optimization-level changes. The PassBuilder C API (`LLVMRunPasses`, `LLVMCreatePassBuilderOptions`) and `LLVMGetHostCPU{Name,Features}` are already in the linked `libLLVM-19.so` — declared in `src/llvm.nuch`, no Makefile/link change needed.
