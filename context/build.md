...
- The compiler emits **opaque-pointer** LLVM IR (`ptr`, not `i8*`/`i32*`). The target triple + datalayout come from LLVM at startup (host by default; overridable with `--target=<triple>`), so emitted IR runs on whatever host LLVM was built for. Backends for non-host triples must be registered before they can be selected — only the X86 backend is currently initialized (`target-init` in `src/nucleusc.nuc`), so `--target=x86_64-apple-darwin` and `--target=x86_64-pc-windows-msvc` work for IR emission but `--target=aarch64-*` does not until Phase B registers more backends.
