...
- The compiler emits **opaque-pointer** LLVM IR (`ptr`, not `i8*`/`i32*`). The emitted `target triple` and pointer size come from a target descriptor set in `compiler-init`: `g-target-triple` (default `"x86_64-pc-linux-gnu"`) and `g-target-ptr-bytes` (default 8, drives type sizes/alignments and `sizeof`'s `size_t` width). To retarget, change those two. The macro/`compile-time` JIT target machine stays the host triple. One spot is still 64-bit-specific: the hand-written `__cons`/`__append` IR in `emit-qq-helpers` (Node size 40, `align 8` on pointer fields).
